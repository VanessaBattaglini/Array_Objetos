...
let primerElemento = radiologia.shift();
console.log(radiologia);

let ultimoElemento = radiologia.pop();
console.log(radiologia);
...
let valoresDental = Object.values(dental);
console.log(valoresDental);

let showDental = dental.forEach(function (d) {
    document.write(`<p>${d.hora} - ${d.especialista}  - ${d.paciente} - ${d.rut} - ${d.prevision}</p>`)
    document.write('***************************************************')

});

// 4. Imprimir un listado total de todos los pacientes que se atendieron en
//     el centro médico.Para esto, deberá unir todos los nombres de 
//     pacientes e imprimir uno por cada párrafo.

let newArray = radiologia.concat(traumatologia, dental);
    console.log(newArray);
    
newArray.forEach(function (p) {
    console.log(p.paciente)
    document.write(`<p>${p.paciente}</p>`);
    document.write('-----------------------------------------');
...
let filtroPrevision = dental.filter(function (paciente) {
    return paciente.prevision === 'ISAPRE';
});
document.write('<h2>Lista de pacientes de consulta dental con previsión ISAPRE</h2>');
document.write('<ul>')
filtroPrevision.forEach(function (paciente) {
...
let valoresTraumatologia = Object.values(traumatologia);
console.log(valoresTraumatologia);

let filtroFONASA = traumatologia.filter(function (t) {
    return t.prevision === 'FONASA';
});
document.write('<h2>Lista de pacientes de consulta de traumatología con previsión FONASA</h2>');
document.write('<ul>')
filtroFONASA.forEach(function (t) {
    document.write(`<li>${t.paciente} - ${t.prevision}</li>`)
    document.write('-----------------------------------------');
});
document.write('</ul>');
